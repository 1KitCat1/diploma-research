...
%---------------------------------------------------------------------------
\begin{abstract}
We extend our prior work on anonymous cross-chain proofs of membership
...
calibrated to our published ECDSA membership circuit.
Third, we present a fresh gnark~v0.14.0 implementation of the ECDSA
membership circuit that reduces R1CS constraints by~74.6\% and proving
key size by~77.1\% relative to our 2023 baseline~\cite{Kravchenko2023}.
...
Beyond the analytical study, we provide fresh empirical benchmarks:
a re-implemented gnark~v0.14.0 circuit that is 74.6\% leaner than
the 2023 baseline, plus open-source Rust benchmark scaffolding for
three additional proof systems (Halo2 via halo2-lib, Bulletproofs
via Spartan, and zk-STARK via RISC~Zero).
...
\textbf{Accumulator} & \textbf{Proof (B)} & \textbf{ZK constr.}
  & \textbf{Trusted setup} & \textbf{Post-quantum}
  & \textbf{Update} & \textbf{Non-membership}
  & \textbf{EVM verify} \\
\midrule
Merkle (Poseidon)        & 640  &   4,280   & No  & Yes & $O(\log n)$ & Hard (sorted) & $O(\log n)$ hashes \\
Sparse Merkle (Poseidon) & 640  &   4,280   & No  & Yes & $O(\log n)$ & Native        & $O(\log n)$ hashes \\
...
We adopt the Sparse Merkle Tree with Poseidon in all subsequent sections:
it supports both membership and non-membership proofs, requires no
trusted setup, is post-quantum secure under collision resistance of
Poseidon, and imposes minimal ZK overhead.
...
is published alongside this paper; full empirical results will be reported
once hardware runs complete.

Table~\ref{tab:zkschemes} reports results at 300,000~R1CS constraints.
\textbf{Groth16} remains optimal for production use: it achieves the
...
This prover-time advantage compounds the gas advantage, making Groth16
the dominant choice for interactive protocols where prover latency is
user-perceptible.
...
We implement a fresh ECDSA membership circuit in gnark~v0.14.0~\cite{gnark}
...
A comparison of five proof systems shows that Groth16 minimises all
three of proof size (192~B), verifier gas (261k), and prover latency
for our circuit.
PLONK is the preferred alternative when trusted setup is unacceptable.
